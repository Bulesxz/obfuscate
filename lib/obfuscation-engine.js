...
class ObfuscationEngine {
    constructor(targetDir, options = {}) {
        ...
        this.options = {
            keywordObfuscationRate: options.keywordObfuscationRate || 0.4,
            enableKeywordObfuscation: options.enableKeywordObfuscation !== false,
            ...options
        };
        ...
        if (this.options.enableKeywordObfuscation) {
            ...
            // 设置关键词混淆率
            this.keywordObfuscator.setObfuscationRate(this.options.keywordObfuscationRate);
            console.log(`🔤 关键词混淆已启用，混淆率: ${(this.options.keywordObfuscationRate * 100).toFixed(1)}%`);
        }
    ...
    async processFiles() {
        ...
        for (const file of files) {
            ...
            // 只处理文本文件，跳过二进制文件
            if (!['.html', '.htm', '.css', '.js', '.ts', '.tsx'].includes(ext)) {
