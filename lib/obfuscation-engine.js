...
class ObfuscationEngine {
    ...
    async run() {
        ...
        // 2. 收集全局CSS映射关系（只在多副本时收集一次，所有副本共享）
        if (this.options.enableCssObfuscation && this.workDirs.length > 1) {
            console.log('🔗 收集全局CSS类名和ID映射（多副本共享）...');
            await this.collectGlobalCssMappings(this.workDirs[0]); // 使用第一个工作目录作为模板
        }
        ...
        for (let i = 0; i < len; i++) {
            ...
            try {
                ...
                // 4. 执行混淆
                const useGlobalMapping = this.workDirs.length > 1; // 多副本时使用全局映射
                await this.processFiles(workDir, !useGlobalMapping); // 单副本时重新收集，多副本时使用全局映射
    ...
    async collectGlobalCssMappings(workDir) {
        console.log('🔍 开始收集全局CSS映射关系...');
        ...
        console.log('✅ 全局CSS映射关系收集完成');
        console.log(`📊 收集到 ${this.mapping.classes.size} 个CSS类名, ${this.mapping.ids.size} 个ID`);
    }

    async processFiles(workDir, collectCssMappings = true) {
        console.log('🚀 开始处理文件混淆...');

        const files = await walkDir(workDir);

        // 第一步：收集所有HTML和CSS文件的类名和ID映射（如果需要）
        if (this.options.enableCssObfuscation && collectCssMappings) {
        ...
        } else if (this.options.enableCssObfuscation && !collectCssMappings) {
            console.log('⏭️ 跳过CSS类名和ID映射收集（使用全局映射）');
        } else {
