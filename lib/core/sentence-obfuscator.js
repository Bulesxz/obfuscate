...
class SentenceObfuscator {
    constructor() {
        this.debug = false;
        this.obfuscationRate = 0.6; // 默认60%概率混淆句子
        this.sentenceMinLength = 10; // 最小句子长度
        this.sentenceMaxLength = 200; // 最大句子长度，避免处理过长内容
    }
            ...
            $('*').each((i, elem) => {
                ...
                directTextNodes.each((j, textNode) => {
                    ...
                    if (trimmedText && 
                        ...
                        // 检查是否应该混淆此句子
                        if (RandomUtils.randomBool(this.obfuscationRate)) {
    ...
    obfuscateTextNode(text) {
        // 随机选择混淆方法 - 只使用span标签和不可见字符，不改变句子内容
        const methods = ['splitWithSpans', 'insertInvisibleChars', 'mixedObfuscation'];
        const method = methods[RandomUtils.randomInt(0, methods.length - 1)];

        switch (method) {
            case 'splitWithSpans':
                return this.splitTextWithSpans(text);
            case 'insertInvisibleChars':
                return this.insertInvisibleChars(text);
            case 'mixedObfuscation':
                return this.mixedTextObfuscation(text);
            default:
                return this.splitTextWithSpans(text);
        }
    }
    ...
    insertInvisibleChars(text) {
        ...
        const chars = [...trimmedText];
        const insertProbability = 0.2; // 20%的位置插入不可见字符
        let result = '';
