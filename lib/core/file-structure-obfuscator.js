...
class FileStructureObfuscator {
    ...
    try {
      ...
      if (allMappings.files && Object.keys(allMappings.files).length > 0) {
        for (const [originalPath, newPath] of Object.entries(allMappings.files)) {
          ...
          // 2. CSS 中的 url() - 改进：支持多种路径格式
          // 匹配完整的文件路径，包括相对路径和绝对路径
          const fileName = path.basename(originalPath);
          ...
          const cssUrlPatterns = [
            // 完整路径匹配
            new RegExp(`(url\\s*\\(\\s*["']?)([^"']*?)(?:${escapedOriginalPath})(["']?\\s*\\))`, 'gi'),
            // 文件名匹配（更宽松的匹配）
            new RegExp(`(url\\s*\\(\\s*["']?)([^"']*?)(?:${escapedFileName})(["']?\\s*\\))`, 'gi'),
            // 相对路径匹配
            new RegExp(`(url\\s*\\(\\s*["']?)([^"']*?)(?:font/[^"']*?${escapedFileName})(["']?\\s*\\))`, 'gi'),
          ];
          
          for (const pattern of cssUrlPatterns) {
            const newContent = content.replace(pattern, (match, prefix, pathPrefix, suffix) => {
              // 检查是否已经包含新路径，避免重复更新
              if (pathPrefix.includes(newPath) || pathPrefix.includes(path.basename(newPath))) {
                return match; // 已经更新过，保持不变
              }
              
              // 计算相对路径
              const relativePath = this.calculateRelativePath(filePath, newPath, projectPath);
            ...
            if (newContent !== content) {
              content = newContent;
              updated = true;
              console.log(`✅ 更新CSS中的路径引用: ${originalPath} -> ${newPath}`);
              break; // 找到匹配就停止，避免重复更新
            }
          }
  ...
  generateObfuscatedFileName(originalName, extension) {
    ...
    if (extension.toLowerCase() === '.css') {
      console.log(`⏭️ 保持CSS文件名: ${originalName}`);
      return originalName;
    }
  ...
  shouldRenameFile(fileName) {
    ...
    // 不重命名CSS文件
    if (ext === '.css') {
      console.log(`⏭️ 跳过CSS文件重命名: ${fileName}`);
      return false;
    }
    
    // 静态资源文件可以重命名（排除CSS）
    const renameableExtensions = ['.js', '.png', '.jpg', '.jpeg', '.gif', '.svg', '.ico', '.woff', '.woff2', '.ttf'];
  ...
  // 计算相对路径
  calculateRelativePath(filePath, newPath, projectPath) {
    try {
      // 获取文件相对于项目根目录的路径
      ...
      const newFileDir = path.dirname(newRelativePath);
      
      // 计算从文件目录到新文件目录的相对路径
      const relativePath = path.relative(fileDir, newFileDir);
      ...
      // 返回相对路径 + 文件名
      return path.join(relativePath, path.basename(newPath)).replace(/\\/g, '/');
    } catch (error) {
      console.warn(`❌ 计算相对路径失败: ${error.message}`);
      // 如果计算失败，返回原始路径
      return newPath;
    }
  }
