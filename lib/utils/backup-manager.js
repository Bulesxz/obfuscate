...
class BackupManager {
    constructor(options = {}) {
        const { targetDir, outputDir = '.', outCount = 1, version = 'v1.0' } = options;
        this.version = version; // 混淆器版本
        this.targetDir = targetDir;
        this.outCount = outCount;
        // 如果输出目录是当前目录，则使用目标目录的父目录(避免直接放目标目录造成循序等)；否则使用指定的输出目录
        this.outputDir = outputDir === '.' ? path.dirname(targetDir) : outputDir;
        this.dirMark = `${path.basename(targetDir)}_${version}_OBF_DIR`; // 混淆目录标记
        ...
        for (let i = 1; i <= outCount; i++) {
            const workDir = path.join(this.outputDir, `${this.dirMark}${i}`);
    ...
    async deleteDirByPrefix() {
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (stats.isDirectory() && item.startsWith(`${this.dirMark}`)) {
            ...
            console.log(`✅ 已删除 ${outputDir} 中所有以 "${this.dirMark}" 开头的目录`);
        } catch (err) {
